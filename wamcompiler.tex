...
\begin{fmpage}{0.9\textwidth}
\verb|(load "average.lisp")|\keys{Enter}\\
\verb|T|\\
...
\section{Roswell}\index{roswell!Installation}
...
kept the \keys{Ctrl} key down and pressed \verb|x|
followed by \verb|s|).
\begin{verbatim}
;; File: celsius.lisp
...
\keys{Alt} key down and presses \verb|x|, then the
...
\end{quote}\index{Predicates!integerp}
...
\begin{quote}\index{Type!class-of}
\begin{verbatim}
* (class-of 10)
...
\begin{description}
\item[integer ---] Integer numbers between $-2147483648$ and $2147483647$. \index{Predicates!integerp}
...
and \verb|nil| otherwise.\index{Predicates!stringp}
...
returns \verb| "llo"|\index{Substring!subseq}
\end{itemize}
...
when \verb|x| is a char.\index{Predicates!characterp}
Let \verb|s| be\index{Type!characterp}
...
\verb|T| if \verb|x| is a cons,\index{Predicates!consp}
...
\begin{itemize}
\item\verb|(string= s z)| -- \verb|T|\index{String!string=}
...
\item\verb|(string<= s z)| -- \verb|T| if\index{String!string<=}
...
\begin{fmpage}{0.8\linewidth}
\verb|(load "list-example.lisp")|\keys{Enter}\\
\verb|T|\\
\verb||\\
\verb|(car xor-structure)| \keys{Enter} \\
\verb|OR|\\
\verb|(cdr xor-structure)|\keys{Enter}\\
\verb|((AND A (NOT B)) (AND (NOT A) B))|\\
\verb|(car (cdr xor-structure))|\keys{Enter}\\
\verb|(AND A (NOT B))|\\
\verb|(car (cdr (car (cdr xor-structure)) ))|\keys{Enter}\\
\verb|A|\\
\verb|(car (cdr (cdr (car (cdr xor-structure)) )))|\keys{Enter}\\
\verb|(NOT B)|
\end{fmpage}
...
\begin{fmpage}{0.8\linewidth}
\verb|(load "spqr.lisp")|\keys{Enter}\\
\verb|(S P Q R)|\\
\verb||\\
\verb|(cdr spqr)|\\
\verb|(P Q R)|\\
...
\section{Named-let}
\index{let-binding!named-let}

In Scheme, named-let\index{Named-let}\index{Loop!named-let}
...
\index{Named-let!meaningful name for loop}
...
optional values,\index{Local variables!default values}
...
The first argument of the \index{with-open-file}
\index{Input from file} \verb|with-open-file|
...
\end{verbatim}
\end{quote}
\index{stream! :supersede}\index{stream! :create}
\index{stream! :append}\index{stream! :direction}
\index{stream! :output}

\index{with-open-file! :supersede}
\index{with-open-file! :create}
\index{with-open-file! :append}
\index{with-open-file! :direction}
\index{with-open-file! :output}
%% \appendix
