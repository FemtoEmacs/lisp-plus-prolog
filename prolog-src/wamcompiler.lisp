  ...
  (let ((*standard-input* stream) (exit-flag nil))
	 ...
	 (let ((find-all-solution nil))
	   (handler-bind
		...
		(divide-head-body clause)
		 (case clause-type
		  ((fact rule)
		   (let ((wamcode 
			   (compile-and-optimize 
			    clause head body)))
		     (let ((key (cons (car head) 
				       (arity head))))
		      (if (eq (gethash key 
					 *predicate-type-table*) 
				'builtin)
		       (format *query-io*
			"Builtin pred ~A/~A cannot be redefined.~%"
				    (car key) (cdr key))
			    (progn
			      (if (have-key? key *clause-code-table*)
				  (rplacd (last (gethash key 
						*clause-code-table*))
					  (list (cons (cadr head)
						      wamcode)))
				  (progn
	...
	(to-lisp-object (store (addr+ (cdr obj) 1)))))
      (struct
       (let* ((struct-info (store (cdr obj)))
	      (functor (car struct-info))
	      (arity (cdr struct-info)))
	 (cons *structure*
	       (cons functor
		     (maprange
		      (lambda (i)
			(to-lisp-object
			 (store (addr+ (cdr obj) i)))) 1 
		      (1+ arity)))))))))
...
(defun send-query (query-code)
  ...
  (let ((*register-area* (make-array 10  :adjustable t))
	...
	*P* *CP* *S* *HB* *H* *B0* *B* *E* *TR* *fail* 
	(*num-of-args* nil) *mode*)
    (setq *P* query-code)
    (setq *CP* nil)
		 ...
		 (case (car inst)
		   ...
		   (get-structure
		    (let* ((f (cadr inst)) (a (caddr inst))
		      ...
		      (case (car (store addr))
			(ref (setf (heap *H*) 
				   (cons 'struct (addr+ *H* 1)))
			     (setf (heap (addr+ *H* 1)) f)
			     (bind addr *H*)
			     (setf *H* (addr+ *H* 2))
			     (setq *mode* 'write))
			(struct 
			  (let ((struct-addr (cdr (store addr))))
		   ...
		   (try-me-else
		    (let* ((l (third inst))
			;; (second inst): the symbol of
			;; the label (for print)
			;; (third inst) : the pointer to
		   ...
		   (trust-me (let ((n (stack *B*)))
				 ...
				 (setf (register (1+ i))
			       ...
			       (setf *E* (stack (addr+ *B* n 1)))
			       (setf *CP* (stack (addr+ *B* n 2)))
			       (unwind-trail (stack (addr+ *B* n 5)) 
					     *TR*)
			       (setf *TR* (stack (addr+ *B* n 5)))
			       (setf *H* (stack (addr+ *B* n 6)))
			       (setf *B* (stack (addr+ *B* n 3)))
			       (setf *HB* (stack (addr+ *B* 
							(stack *B*) 
							6)))
			       (setq *P* (cdr *P*))))
		   (try
		   ...
		   (trust (let ((l (third inst))
			    ...
			    (setf *P* l)))
		   (switch-on-term (let ((v (sixth inst)) 
					 (c (seventh inst))
					 (l (eighth inst)) 
					 (s (ninth inst)))
				     (let ((ptr 
